...
case class ScTypePolymorphicType(internalType: ScType, typeParameters: Seq[TypeParameter])
                                (implicit val typeSystem: TypeSystem) extends NonValueType {
  ...
  override def presentableText = typeSystem.presentableText(this)

  override def canonicalText = typeSystem.canonicalText(this)
